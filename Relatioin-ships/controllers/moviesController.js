...
const getMoviesById = async (req, res) => {
    ...
    try {
        const movie = await movieModel.findById(movieId, { name : 1, rating : 1, _id : 0 }).populate('reviews', { userName : 1, comment : 1, _id : 0 });
        if(!movie) {
...
const deleteMoviesById = async (req, res) => {
    ...
    try {
        ...
        res.send({status : 'success', msg : "Deleted successfully", movie : deletedMovie});
    } catch (err) {
